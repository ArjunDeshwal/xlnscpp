...
//    modified for 16-bit similar to bfloat (see xlns32.cpp for original
//    float-like code) with the xlns16_ideal option with a Mitchell LPVIP method
//    for the non-ideal case (Apr'25 few sml err/bugs vs python lpvip) with
//    xlns16_alt for streamlined + for modern arch w/ ovfl test with
//    xlns16_table for fast table lookup conversion (fast sb db if xlns_alt and
//    not ideal)
//      each cvt table 256Kbyte; sb db table each 5Kbyte
// they are based on similar math foundation (Gaussian logs, sb and db) as
// Python xlns,
...
#define xlns16_sqrt(x)                                                         \
  (xlns16_abs(((xlns16_signed)((x) << 1)) / 4) ^ xlns16_sqrtmask)
#define xlns16_canon(x) ((x) ^ (-((x) >> xlns16_canonshift) | xlns16_signmask))

inline xlns16 xlns16_overflow(
    xlns16 xlns16_x, xlns16 xlns16_y,
    xlns16 xlns16_temp) { // printf("%d %d %d\n",xlns16_temp,xlns16_x,xlns16_y);
...
inline xlns16 xlns16_mul(xlns16 x, xlns16 y) {
  ...
  // xlns16_y=y;
  xlns16_temp =
      (xlns16_logmask & (x)) + (xlns16_logmask & (y)) - xlns16_logsignmask;
  return (xlns16_signmask & (xlns16_temp))
             ? xlns16_overflow(x, y, xlns16_temp)
...
inline xlns16 xlns16_sb_ideal(xlns16_signed z) {
  return (
      (xlns16)(log2(1 + exp2(((double)z) / xlns16_scale)) * xlns16_scale + .5));
}
inline xlns16 xlns16_db_ideal(xlns16_signed z) {
  return ((xlns16_signed)(log2(exp2(((double)z) / xlns16_scale) - 1) *
                              xlns16_scale +
                          .5));
}
#else
...
{
  return ((xlns16_signed)((log(pow(2.0, ((double)z) / xlns16_scale) - 1) /
                           log(2.0)) *
                              xlns16_scale +
                          .5));
}
inline xlns16 xlns16_mitch(xlns16 z) {
  return (((1 << xlns16_F) + (z & ((1 << xlns16_F) - 1))) >>
          (-(z >> xlns16_F)));
}

inline xlns16 xlns16_sb_premit_neg(
    xlns16_signed zi) // was called premitchnpi(zi): assumes zi<=0
{
  xlns16 postcond;
  xlns16 z;
  postcond =
      (zi <= -(3 << xlns16_F)) ? 0 : (zi >= -(3 << (xlns16_F - 2)) ? -1 : +1);
  z = ((zi << 3) + (zi ^ 0xffff) + 16) >> 3;
  return (zi == 0) ? 1 << xlns16_F : xlns16_mitch(z) + postcond;
  // return ((zi==0)?1<<xlns16_F:
  // (((1<<xlns16_F)+(z&((1<<xlns16_F)-1)))>>(-(z>>xlns16_F)))+postcond );
}

inline xlns16 xlns16_db_premit_neg(xlns16_signed z) // assumes zi<0
{
  xlns16_signed precond;
  precond = (z < -(2 << xlns16_F)) ? 5 << (xlns16_F - 3) : //  0.625
                (z >> 2) + (9 << (xlns16_F - 3));          //  .25*zr + 9/8
  return (-z >= 1 << xlns16_F)
             ? -xlns16_mitch(z + precond)
...
inline xlns16 xlns16_add(xlns16 x, xlns16 y) {
  ...
  adjust =
      z + ((xlns16_signed)(log(pm1 + pow(2.0, -((double)z) / xlns16_scale)) /
                               log(2.0) * xlns16_scale +
                           .5));
  ...
  xlns16_signed postcond = (z <= -(3 << xlns16_F)) ? 0
                           : z >= -(3 << (xlns16_F - 2))
                               ...
                               : +(1 << (xlns16_F - 6));
  xlns16_signed mitch =
      (-z >= 1 << xlns16_F) || (usedb == 0)
          ? xlns16_mitch(z + precond)
          : -xlns16_db_ideal(-z) - z; // use ideal for singularity
  adjust = usedb ? -mitch : (z == 0) ? 1 << xlns16_F : mitch + postcond;
#else
  adjust = usedb ? xlns16_db_premit_neg(z) : xlns16_sb_premit_neg(z);
#endif
  adjustez = (z < -xlns16_esszer) ? 0 : adjust;
#endif
#endif
...
inline float xlns162fp(xlns16 x) { return xlns16cvtbl[x]; }
...
xlns16 fp2xlns16(float x) {
  if (x == 0.0)
    ...
    return (((xlns16_signed)(log2(fabs(x)) * xlns16_scale)) | xlns16_signmask) ^
           xlns16_logsignmask;
}
...
float xlns162fp(xlns16 x) {
  if (xlns16_abs(x) == xlns16_zero)
    ...
    return (float)(-pow(
        2.0, ((double)(((xlns16_signed)(xlns16_abs(x) - xlns16_logsignmask)))) /
                 ((float)xlns16_scale)));
  else {
    return (float)(+pow(
        2.0, ((double)(((xlns16_signed)(xlns16_abs(x) - xlns16_logsignmask)))) /
...
xlns16_float float2xlns16_(float y) {
  ...
  if ((xlns16_cachetag[addr] == y) && xlns16_cacheon) {
    ...
    // fpbyte[0],fpbyte[1],fpbyte[2],fpbyte[3],addr);
