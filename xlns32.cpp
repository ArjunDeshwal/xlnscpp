...
// they were modified with xlns32_alt for streamlined + on modern arch w/ ovfl
// detect they are based on similar math foundation (Gaussian logs, sb and db)
// as Python xlns,
...
//    for the 32-bit format in this file, this is roughly similar to IEEE 754
//    float32 there is an exact representation of 0.0, but no subnormals or NaNs
...
#define xlns32_sqrt(x)                                                         \
  (xlns32_abs(((xlns32_signed)((x) << 1)) / 4) ^ xlns32_sqrtmask)
#define xlns32_canon(x) ((x) ^ (-((x) >> xlns32_canonshift) | xlns32_signmask))

inline xlns32
xlns32_overflow(xlns32 xlns32_x, xlns32 xlns32_y,
                xlns32 xlns32_temp) { // printf("%ld %ld
...
inline xlns32 xlns32_mul(xlns32 x, xlns32 y) {
  ...
  // xlns32_y=y;
  xlns32_temp =
      (xlns32_logmask & (x)) + (xlns32_logmask & (y)) - xlns32_logsignmask;
  return (xlns32_signmask & (xlns32_temp))
             ? xlns32_overflow(x, y, xlns32_temp)
...
inline xlns32 xlns32_sb_ideal(xlns32 z) {
  return (
      (xlns32)(log2(1 + exp2(((double)z) / xlns32_scale)) * xlns32_scale + .5));
}
inline xlns32 xlns32_db_ideal(xlns32 z) {
  return ((xlns32_signed)(log2(exp2(((double)z) / xlns32_scale) - 1) *
                              xlns32_scale +
                          .5));
}
#else
...
#define xlns32_sb_macro(z)                                                     \
  (((xlns32_zh = (xlns32_z = (z)) >> xlns32_zhshift) >=                        \
    (xlns32_tablesize - 1))                                                    \
       ? xlns32_z                                                              \
       : (((xlns32_z & xlns32_zhmask) +                                        \
           (((xlns32)xlns32_sbhtable[xlns32_zh]) << 16) +                      \
           xlns32_sbltable[xlns32_zh]) +                                       \
          (((0x4000 - (xlns32_zlmask & ((xlns32_sbltable[xlns32_zh] -          \
                                         xlns32_sbltable[xlns32_zh + 1])))) *  \
...
xlns32 xlns32_dbtrans3(xlns32 z) {
  ...
  if (z1 == 0) {
    if (z2 == 0) {
      ...
      return xlns32_db2table[z2] +
             xlns32_sb(z2 + xlns32_db0table[z0 >> xlns32_db0shift] -
                       xlns32_db2table[z2]);
  } else if (z2 == 0) {
    if (z0 == 0)
      ...
      return xlns32_db1table[z1 >> xlns32_db1shift] +
             xlns32_sb(z1 + xlns32_db0table[z0 >> xlns32_db0shift] -
                       xlns32_db1table[z1 >> xlns32_db1shift]);
  } else {
    if (z0 == 0)
      return xlns32_db2table[z2] +
             xlns32_sb(z2 + xlns32_db1table[z1 >> xlns32_db1shift] -
                       xlns32_db2table[z2]);
    else {
      temp2 = xlns32_sb(z1 + xlns32_db0table[z0 >> xlns32_db0shift] -
                        xlns32_db1table[z1 >> xlns32_db1shift]);
      return xlns32_db2table[z2] +
             xlns32_sb(z2 + xlns32_db1table[z1 >> xlns32_db1shift] + temp2 -
                       xlns32_db2table[z2]);
    }
  }
}
...
inline xlns32 xlns32_add(xlns32 x, xlns32 y) {
  ...
  // adjust = log(1.0 +
  // pm1*pow(2.0,((double)z)/xlns32_scale))/log(2.0)*xlns32_scale+.5;
  adjust =
      z + ((xlns32_signed)(log(pm1 + pow(2.0, -((double)z) / xlns32_scale)) /
                               log(2.0) * xlns32_scale +
                           .5));
#else
...
xlns32 fp2xlns32(float x) {
  if (x == 0.0)
    ...
    return (((xlns32_signed)(log2(fabs(x)) * xlns32_scale)) | xlns32_signmask) ^
           xlns32_logsignmask;
}
...
float xlns322fp(xlns32 x) {
  if (xlns32_abs(x) == xlns32_zero)
    ...
    return (float)(-exp2(
        ((double)(((xlns32_signed)(xlns32_abs(x) - xlns32_logsignmask)))) /
        ((float)xlns32_scale)));
  else {
    return (float)(+exp2(
        ((double)(((xlns32_signed)(xlns32_abs(x) - xlns32_logsignmask)))) /
...
xlns32 xlns32_internal(xlns32_float y) { return y.x; }

float xlns32_2float(xlns32_float y) { return xlns322fp(y.x); }
